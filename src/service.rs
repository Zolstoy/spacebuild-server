    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    async fn handle_message_for_auth(&mut self, message: Message) -> Result<(Sender<Action>, Receiver<State>)> {
        match message {
            Message::Text(msg) => {
                let maybe_action: serde_json::Result<Action> = serde_json::from_str(msg.as_str());

                let mut auth_info = AuthState {
                    success: false,
                    message: "".to_string(),
                };
    ...
    async fn handle_message_for_gameplay(&mut self, send: Sender<Action>, recv: Receiver<State>) -> Result<()> {
